...
export default function Body() {
    ...
    //=> Ref 객체를 생성하고 textRef에 저장
    //=> input Tag에서 ref={textRef} 정의하면
    //  textRef는 DOM 엘리먼트에 접근하도록 설정되고
    //  textRef를 이용해서 DOM 엘리먼트를 직접 조작 가능
    ...
    const onClickBtn = ()=>{
        //=> Test1)
        // alert(`** 버튼클릭: text=${text}`);
        // textRef.current.value='So difficult...';
        // console.log(`** 버튼클릭 textRef 변경 후: text=${text}`);
        //=> 이 값은 DOM 엘리먼트에 직접 반영
        //  그래서 화면 출력은 되지만 랜더링은 일어나지 않았고, 상태변수에도 영향을 주지 않음
        //=> document.getElementById('...').value 접근과 동일
        
        //=> Test2) 포커스에 적용
        //  입력된 text길이가 3 미만이면 포커스가 머문 상태로 입력을 기다리도록
        if(text.length<3) {
            alert(`** 3글자 이상 입력하세요`);
            ...
            console.log(`** textRef_value clear : text=${text}`);
        }        
    } //onClickBtn
