...
//    const [text_State변수, setText_set함수] = useState("State변수의 초기값");
...
export default function Body() {
    ...
    //1) 정의 & 출력
    //1.1) 일반변수 
    //=> 값이 변경되어도 화면에는 변화 없음(console에는 반영됨)
    //let count=0;

    //1.2) 상태변수
    //=> 상태변수의 값은 두번째 요소인 set함수로 해야함
    const [count, setCount] = useState(0);

    const onIncrease = ()=>{
        //count++; -> 상태변수는 직접 변경 불가 set함수로만 가능
    ...
    // => check 변수를 추가하고 1씩 증가 시키면서, 5회 증가하면
    ...
    const onIncrease2 = ()=>{
        ...
        if(check%5==0) {
            ...
            //      그러므로 모든 함수내의 지역변수는 초기화 됨
            //      그러나 상태변수는 초기화되지 않음(값을 유지함)
            console.log(`** onIncrease2 if setCount 전 check=${check}`);
            //setCount(check); -> check가 0값으로 초기화되고 담겨지므로 count 값도 0
            setCount(count+5);
            console.log(`** onIncrease2 if setCount 후 count=${count}`);
        }
    } //onIncrease2
    ...
    //=====================================================================
    // ** 다양한 input Tag
    //=> input type text, textarea등... : value 속성값으로 입력값 전달
    //=> 상태변수와 이벤트핸들러가 필요함
    ...
    //=> date
    //  date 엘리먼트의 입력값을 console로 출력하세요
    ...
    //=> select
    // -> html 과 차이점
    //    selected 속성 대신 value를 사용해 기본값 할당. (아래코드에서는 애플)
    ...
    return (
        <div className="body">
            ...
            {/* <input /><span>가나다라</span> 
                => 비교구문
                - text라는 변수의 영향을 받지 않는 코드이므로 입력갑이 표시됨 */}
            <input value={text} onChange={textChange} />&nbsp;&nbsp;<span>{text}</span>
            {/* => 키보드 입력하도 화면에 표시되지 않음
                => 키보드 누를 때마다 text 변수의 값을 변경시켜야 하고(이벤트코드 필요) 
                    출력되게 하려면 랜더링이 일어나야함(상태변수 필요)  */}
            <br /><br />
