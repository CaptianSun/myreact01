...
function App() {
    // ** 1. Data 전달(props, event 활용)
    //=> 함수도 Child로 전달 가능
    //  Child에서 발생한 이벤트를 부모로 전달 가능함
    ...
    // ** 2. text 추가 후 랜더링 비교
    //=> Viewer, Controller와 무관한 상태변수
    //=> 결론: 부모에서 랜더링이 일어나면 자식도 무조건 랜더링됨
    //=> 랜더링 관리가 필요함
    //  함수형 컴포넌트의 LifeCycle 관리가 필요하고, useEffect로 가능
    const [text, setText] = useState('');
    const onChangeText = (e)=>{setText(e.target.value);}

    // ** 3. 컴포넌트 LifeCycle과 useEffect
    /*3.1) 정의 Test, 상태변수 count
    useEffect(()=>{
        console.log(`** useEffect 3.1) count=${count}, text=${text}`)
    }, [count, text]);
    */
    //3.2) 상태변수 text 추가 후 비교
    //=> [count] : count 값 변화에만 반응
    //=> [text] : text 값 변화에만 반응
    //=> [count, text] : count, text 값 변화에만 반응
    ...
    */
    ...
    /*3.4) 두번째인자가 빈 배열(LifeCycle01, Mount제어)
    //=> 첫 랜더링때만 호출되므로 Mount 제어에 이용함
    useEffect(()=>{
        console.log(`** useEffect 3.4) count=${count}, text=${text}`);
    }, []);
    

    //3.5) 리랜더링 시에만 호출(LifeCycle02, Update제어)
    //=> "3.3) 랜더링 할 때마다 호출" 에서 "3.4) 첫 랜더링때만 호출"의 경우를 제외시켜줌
    //=> 최초 랜더링 인지를 확인 후 아닌 경우에만 실행되도록 콜백함수를 작성
    //=> 콜백함수에서
    //  -> 최초 랜더링(마운트) 시점 인지 판별하는 변수를 정의하고
    //     초기값을 false 로 지정, 이때 이 변수는 Ref 객체로 생성
    //     왜냐하면 랜더링 할때마다 이 변수 값이 초기화 되면 안되고, 
    //     이 변수로 인해 랜더링이 일어나지 않도록 하기 위함
    //  -> callback_함수에서는
    ...
    useEffect(()=>{
        if(!didMountRef.current) 
            //=> didMountRef.current 값이 true가 아니면
            //=> Ref 객체는 current 속성으로 접근(const didMountRef = true 이 아님)
    ...
    //=> 실행결과
    //  -> setInterval 의 콜백함수는 2초 단위로 실행되어야 하지만, 점점 빨라짐
    //  -> 랜더링 할때마다 이전에 호출되었던 함수가 남아있는 상태에서
    //    계속 호출되었기 때문에 복수의 setInterval 이 계속 생성되기 때문
    //  -> 이를 방지하려면 새로운 호출 전에 사용을 마친 setInterval 은 종료시켜야함
    //  -> setInterval 의 종료함수는 clearInterval
    //  -> 해결: useEffect 의 클린업 기능
    
    //=> 클린업 함수
    //  -> useEffect 의 콜백함수가 return 하는 함수
    //  -> 컴포넌트를 언마운트하기 전 또는 콜백함수를 재호출하기 전에 실행됨
    //  -> 그러므로 이를 이용하여 리랜더링 할 때마다 새로운 setInterval 을 생성하고
    ...
    */
    ...
    return (
        <div className="App">        
            <h3>** Simple Counter와 함수컴포넌트의 LifeCycle **</h3>
            <section>
                <Viewer count={count} />
                {/* 3.6.2) Test 
                    => count값이 짝수일 때만 Even 출력*/}                
                {count%2===0 && <Even></Even>}
                {/* && : 앞쪽 조건식이 참이면 뒤쪽 리턴값을 랜더링
                        (만약 거짓이면 아무것도 랜더링 하지 않음)
                    || : 앞쪽 조건식이 거짓이면 뒤쪽 리턴값을 랜더링 */}
