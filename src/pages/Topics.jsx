...
//2.1) 메뉴 Link 목록을 배열로 
const contents = [
    {id:'1', title:'Join', description:'회원가입을 하시고 더 많은 정보를 만나보세요'},
    {id:'2', title:'Login', description:'로그인 되었습니다 반갑습니다'},
    {id:'3', title:'Board', description:'이곳은 자유게시판입니다'},
    {id:'4', title:'Myinfo', description:'정보를 확인하세요'},
    {id:'5', title:'공지사항', description:'공지사항입니다'}
];
...
function Topic() {
    ...
    //2) 배열(contents)에서 id값이 topic_id와 일치하는 자료 찾기
    //=> filter 적용
    ...
    //    if(find_item == undefined) 와 같은 구문은 성립될 수 없고 오류임 주의
    const find_item = contents.find(({id})=>id===topic_id);

    //=> 초기값을 Not Found로 정의
    //  find_item 값이 있으면 selected_item에 담아서 전달
    let selected_item = {
        title:'Sorry', 
...
export default function Topics() {
    ...
    return (
        <div>
            ...
            {/* Router 에 적용 
                -> 현재 반 자동화
                <Route path="/1" element={'회원가입을 하시고 더 많은 정보를 만나보세요'} />
                ......
                <Route path="/4" element={'정보를 확인하세요'} />
                -> 자동화 
                    - Route 도 동일구조 이므로 하나의 Route로 처리
                    - path 의 값(id)만 전달받으면 해당하는 컨텐츠를 출력하도록
                    - id에 해당하는 컨텐츠를 찾는 코드가 필요함
                    - 이 역할을 하위 컴포넌트(Topic)로 작성함
                -> path(1,2,3,4)의 변수화
                    - ":변수명", 이것을 Parameter 라고 함
                    - url의 일부인지 아니면 Parameter인지 구별하기 위해 ":"을 붙임
                    - 이 Parameter 의 값은 옆의 element 속성에 정의된 컴포넌트(Topic)로 전달됨
                    - 컴포넌트(Topic) 에서는 useParams() 사용함
                */}
            <Routes>
    ...
    return (
        <div>
            ...
            {/* 하위메뉴 추가
                ...
                - url 설정, url과 view(컴포넌트)를 연결
                - 상위 컴포넌트에서 /topics/* (하위메뉴)에 대한 Route 설정 필요 */}
            <ul>
            ...
            <Routes>
                <Route path="/join" element={'회원가입을 하시고 더 많은 정보를 만나보세요'} />
                <Route path="/login" element={'로그인 되었습니다 반갑습니다'} />
                <Route path="/board" element={'이곳은 자유게시판입니다'} />
            </Routes>
        </div>
    ); //return
}